...
def getFoodTrucksByApplicantName(applicant_name, record = 5):
    res = []
    try:
        query = """SELECT locationid, applicant, facilityType, locationDescription, address, status, foodItems, ExpirationDate FROM facility ORDER BY SIMILARITY(applicant, %s) DESC LIMIT %s;"""
        __execute_querry(res, query, (applicant_name, record,))
        return res
...
def getExpiredFoodTrucks(curr_date):
    res = []
    try:
        query = """select * from facility where expirationdate < %s;"""
        __execute_querry(res, query, (curr_date,))
        return res
    except Exception as e:
        print("Something went wrong", e)
        return res
...
def getFoodTrucksByStreet(street):
    res = []
    try:
        query = """select locationid, applicant, locationdescription, address from facility where address_with_weights @@ to_tsquery(%s);"""
        __execute_querry(res, query, (street,))
        return res
    except Exception as e:
        print("Something went wrong", e)
        return res
...
def getNearestFoodTrucks(latitude, longitude):
    res = []
    try:
        query = """SELECT *, ST_Distance(location, ST_SetSRID(ST_MakePoint(%s, %s), 4326)) FROM facility ORDER BY location <-> ST_SetSRID(ST_MakePoint(%s, %s), 4326) limit 10;"""
        __execute_querry(res, query, (latitude, longitude, latitude, longitude,))
        return res
    except Exception as e:
        print("Something went wrong", e)
        return res
